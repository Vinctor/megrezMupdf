...
FUN(PDFDocument_getMetaData)(JNIEnv *env, jobject self)
{
	fz_context *ctx = get_context(env);
	pdf_document *pdf = from_PDFDocument(env, self);
	pdf_obj *xml = NULL;
	...
	fz_try(ctx){
				xml = pdf_dict_getl(ctx, pdf_trailer(ctx, pdf), PDF_NAME(Root), PDF_NAME(Metadata), NULL);
				if(!xml) return NULL;
				buff = pdf_load_stream(ctx, xml);
				info = fz_string_from_buffer(ctx,buff);

			}
	fz_always(ctx){
			fz_drop_buffer(ctx,buff);
			pdf_drop_obj(ctx,xml);
		}
	fz_catch(ctx)
		jni_rethrow(env, ctx);
...
JNIEXPORT void JNICALL
FUN(PDFDocument_setMetaData)(JNIEnv *env, jobject self, jstring jcontents)
{
    fz_context *ctx = get_context(env);
    pdf_document *pdf = from_PDFDocument(env, self);
    ...
    if (!ctx || !pdf) return ;
	if (!jcontents) jni_throw_arg_void(env, "contents must not be null");
    ...
    fz_try(ctx){
    	//将contents转为buffer
		buffer = fz_new_buffer_from_copied_data(ctx, (const unsigned char *)scontents, strlen(scontents));
		pdf_obj *root = pdf_dict_get(ctx, pdf_trailer(ctx, pdf), PDF_NAME(Root));
		if(!root) jni_throw_arg_void(env, "root should not be null");
		pdf_obj *xml = pdf_add_stream(ctx, pdf, buffer, NULL, 0);
		pdf_dict_put(ctx, xml, PDF_NAME(Type), PDF_NAME(Metadata));
		pdf_dict_put(ctx, xml, PDF_NAME(Subtype), PDF_NAME(XML));
		pdf_dict_put_drop(ctx, root, PDF_NAME(Metadata), xml);
    }
    fz_always(ctx){
