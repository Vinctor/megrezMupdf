...
JNIEXPORT void JNICALL
FUN(PDFAnnotation_setNativeAppearanceImage)(JNIEnv *env, jobject self, jstring jappearance, jstring jstate, jobject jbbox, jobject jctm, jobject jimage)
{
    fz_image *image = from_Image_safe(env, jimage);
	fz_rect rect = from_Rect(env, jbbox);
	fz_context *ctx = get_context(env);
	pdf_annot *annot = from_PDFAnnotation(env, self);
    fz_display_list *dlist = fz_new_display_list(ctx, rect);
	fz_var(dlist);
	fz_device *dev = fz_new_list_device(ctx, dlist);
	fz_matrix ctm = from_Matrix(env, jctm);
	fz_fill_image(ctx, dev, image, ctm, 1.0, fz_default_color_params);
	...
	fz_try(ctx)
				pdf_set_annot_appearance_from_display_list(ctx, annot, appearance, state, ctm, dlist);
	fz_always(ctx)
		{
			if (jstate)
				(*env)->ReleaseStringUTFChars(env, jstate, state);
